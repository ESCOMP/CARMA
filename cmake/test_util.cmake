...
################################################################################
# build and add a standard test (one linked to the carma library)
...
function(create_standard_test)
  ...
  target_link_libraries(test_${TEST_NAME} PUBLIC musica::carma carma_test_utils GTest::gtest_main)
  if(CARMA_ENABLE_OPENMP)
  ...
  if(CARMA_ENABLE_LAPACK)
    target_link_libraries(test_${TEST_NAME} PUBLIC ${LAPACK_LIBRARIES} ${BLAS_LIBRARIES})
  endif()
  add_carma_test(${TEST_NAME} test_${TEST_NAME} "" ${TEST_WORKING_DIRECTORY})
...
function(create_standard_cxx_test)
  ...
  add_executable(test_${TEST_NAME} ${TEST_SOURCES})
  target_link_libraries(test_${TEST_NAME} PUBLIC musica::carma GTest::gtest_main)
  ...
  add_carma_test(${TEST_NAME} test_${TEST_NAME} "" ${TEST_WORKING_DIRECTORY} ${TEST_SKIP_MEMCHECK})
endfunction(create_standard_cxx_test)

################################################################################
# Add a test

function(add_carma_test test_name test_binary test_args working_dir)
  if(CARMA_ENABLE_MPI)
...
function(add_regression_test test_name command memcheck_command)
  ...
  if(MEMORYCHECK_COMMAND AND CARMA_ENABLE_MEMCHECK)
...
################################################################################
# Link carma to a test and add it to the suite as a bash script

macro(add_std_test_script test_name script_path)
  target_include_directories(${test_name} PUBLIC ${CMAKE_BINARY_DIR}/src)
  target_link_libraries(${test_name} PUBLIC musica::carma)
  if(CARMA_ENABLE_OPENMP)
