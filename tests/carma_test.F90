...
subroutine test_dust_1Dv2()
  ...
  allocate(zc(NZ,NY,NX), zl(NZP1,NY,NX), p(NZ,NY,NX), pl(NZP1,NY,NX), t(NZ,NY,NX))
...
!			write(*,"(i2,': (',i5,',',i5,')')") ithread, iy, ix
	...
	!    write(*,*) "  CARMASTATE_Create"
			call CARMASTATE_Create(cstate(ithread), carma_ptr, time, dtime, NZ, I_CART, lat(iy,ix), lon(iy,ix), &
